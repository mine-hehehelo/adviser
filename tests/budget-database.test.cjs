...
(async()=>{
 ...
 const otherUser=req();
 const active=req(); await begin(active);
 await assert.rejects(db.query('select public.delete_advisor_conversation($1,$2) as deleted',[user,conv]),/conversation_has_processing_turn/);
 await fail(active);
 assert.equal((await db.query('select public.delete_advisor_conversation($1,$2) as deleted',[otherUser,conv])).rows[0].deleted,false);
 assert.equal((await db.query('select public.delete_advisor_conversation($1,$2) as deleted',[user,conv])).rows[0].deleted,true);
 assert.equal((await db.query('select count(*)::int as count from public.conversations where id=$1',[conv])).rows[0].count,0);
 assert.equal((await db.query('select count(*)::int as count from public.messages where conversation_id=$1',[conv])).rows[0].count,0);
 const redacted=(await db.query('select user_input,assistant_response,conversation_id from public.advisor_turn_logs where user_id=$1',[user])).rows;
 assert.ok(redacted.length>0);
 assert.ok(redacted.every(row=>row.user_input==='[deleted conversation]'&&row.assistant_response===null&&row.conversation_id===null));
 assert.equal(await total(),9150);
 assert.equal((await db.query("select has_function_privilege('authenticated','public.delete_advisor_conversation(uuid,uuid)','EXECUTE') as allowed")).rows[0].allowed,false);
 console.log('PASS deletion waits for processing and redacts owned chat without resetting usage');
 await db.close();
